    ...
    <div class="pt-4 bg-white shadow rounded">
      ...
      <div class="p-4 overflow-x-auto">
        <table class="table-auto w-full">
          <thead>
            <tr class="text-xs text-gray-500 text-left"><th class="pb-3 font-medium">ID</th><th class="pb-3 font-medium">Name</th><th class="pb-3 font-medium">Image</th><th class="pb-3 font-medium">skills</th><th class="pb-3 font-medium">Url</th><th class="pb-3 font-medium">Status</th></tr>
          </thead>
          <tbody>
            <tr class="text-xs bg-gray-50" v-for="project in projects.data" :key="project.id">
              <td class="py-5 px-6 font-medium">{{  project.id  }}</td>
              <td class="font-medium">{{  project.name  }}</td>
              <td class="font-medium">
                <img :src="project.image" class="w-12 h-12 rounded-md" />
              </td>
              <td class="font-medium">{{  project.skill.name  }}</td>
              <td class="font-medium">{{  project.project_link   }}</td>
              <td>
                <Link :href="route('projects.destroy',project.id)"   method="delete"
                type="button"
                as="button" class="inline-block py-1 px-2 text-white bg-red-500 rounded-full">Delete</Link>
                <Link :href="route('projects.edit',project.id)"
                class="inline-block py-1 px-2 text-white bg-green-500 rounded-full" >Edit</Link>
              </td>
            </tr>
